...
public class SoundSettingsController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        Platform.runLater(
                () -> {
                    ...
                    if (!(previousUserName.equals(gameSettings.getCurrentUser()))) {
                        ...
                        try {
                            BufferedReader bufferedReader = new BufferedReader(
                                    new FileReader("DATABASE/usersettings/" + previousUserName));
                    ...
                    //Add listeners to both sliders, which alters the ingame volume while the slider is changed
                    ...
                    backgroundMusicSlider.valueProperty().addListener(new InvalidationListener() {
                        ...
                        @Override
                        public void invalidated(Observable observable) {
                            SoundsManager.changeBackgroundMusicVolume(backgroundMusicSlider.getValue() / 100);
    ...
    private void onSoundEffectsDragDetected() {
        //Add listed to sound effect slider, if the slider is moved to a new position, record new settings value and play sfx
    ...
    private void onBackgroundMusicDragDetected() {
        //Add listed to background music slider, if the slider is moved to a new position, record new settings value and play sfx
        backgroundMusicSlider.valueChangingProperty().addListener(new ChangeListener<Boolean>() {
    ...
    private void onMute() throws URISyntaxException, IOException {
        //If the mute status setting is 0, clicking the mute button should mute the game and change mute status to 1
        ...
        //Other wise if mute status is 1, clicking the mute button should unmute the game and change the mute status to 0
